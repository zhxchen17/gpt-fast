...
def _load_model(checkpoint_path, device, precision, use_tp):
    ...
    if "int4-hqq" in str(checkpoint_path):
        ...
        assert path_comps[-4].startswith("g")
        assert path_comps[-3] in device, "weight packed format mismatch, please rerun quantize.py!"
        assert path_comps[-2].startswith("axis")
        groupsize = int(path_comps[-4][1:])
        axis = int(path_comps[-2][4:])
        quantizer = WeightOnlyInt4HqqQuantHandler(model, groupsize=groupsize, axis=axis)
